...
class VaListRequest implements ModelInterface, ArrayAccess
{
    ...
    protected static $swaggerTypes = [
        ...
        'va_status_code_list' => '\Ganb\Corporate\Client\Model\VaStatusCode[]',
     ...
     *
     * @param string $va_contract_auth_key 振込入金口座API認証情報 半角英数字 銀行契約の方はNULLを設定 提携企業の方が、契約された顧客の一覧照会を依頼される場合は必須 提携企業以外の方が値を設定されている場合は「400 Bad Request」を返却
     *
     * @return $this
     */
     ...
     *
     * @return \Ganb\Corporate\Client\Model\VaStatusCode[]
     */
     ...
     *
     * @param \Ganb\Corporate\Client\Model\VaStatusCode[] $va_status_code_list 振込入金口座IDリスト 照会したい振込入金口座状態コードのリスト 上限3件まで設定可能
     *
     * @return $this
     */
     ...
     * Sets expire_date_to
     *
     * @param string $expire_date_to 有効期限To 半角文字 YYYY-MM-DD形式 振込入金口座 種類コードが未設定もしくは、「1＝期限型」が設定されている場合は指定可　それ以外はNULLを設定（値が設定されている場合は、「400 Bad Request」を返却） 有効期限Fromと有効期限Toを指定する場合は、有効期限From≦有効期限Toとし、それ以外は「400 Bad Request」を返却
     *
     * @return $this
     */
     ...
     * Sets va_id_list
     *
     * @param \Ganb\Corporate\Client\Model\VaId[] $va_id_list 振込入金口座IDリスト 照会したい振込入金口座IDのリスト 上限500件まで設定可能
     *
     * @return $this
     */
