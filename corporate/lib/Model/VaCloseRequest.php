...
/**
 * VaCloseRequest Class Doc Comment
 *
 * @category Class
 * @package  Ganb\Corporate\Client
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class VaCloseRequest implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'VaCloseRequest';
     ...
     *
     * @param string $va_contract_auth_key 振込入金口座API認証情報 半角英数字 銀行契約の方はNULLを設定 提携企業の方が、契約された顧客の解約申込を依頼される場合は必須 提携企業以外の方が値を設定されている場合は「400 Bad Request」を返却
     *
     * @return $this
     */
    public function setVaContractAuthKey($va_contract_auth_key)
    {
        if (!is_null($va_contract_auth_key) && (mb_strlen($va_contract_auth_key) > 400)) {
            throw new \InvalidArgumentException('invalid length for $va_contract_auth_key when calling VaCloseRequest., must be smaller than or equal to 400.');
        }
        if (!is_null($va_contract_auth_key) && (mb_strlen($va_contract_auth_key) < 1)) {
            throw new \InvalidArgumentException('invalid length for $va_contract_auth_key when calling VaCloseRequest., must be bigger than or equal to 1.');
        }
